...
// Rebuild relationships from article text so the globe can be safely refreshed after classifier changes.
...
for (const summary of articles) {
  ...
  console.log(
    `${article.headline}: ${assignments.map((item) => item.code).join(", ") || "no country match"}`,
  );
}
await pool.end();
