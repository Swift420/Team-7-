...
const columns = `
  ...
  source_url, published_at, body, raw_content, source_format, teaser_image, tags,
  created_at, updated_at
`;
const summaryColumns = `
  id, import_key, nzz_id, document_id, headline, lead, author_line, section, language,
  source_url, published_at, source_format, teaser_image, tags, created_at, updated_at
`;
...
export async function listArticles(): Promise<ArticleSummary[]> {
  const result = await query<Omit<ArticleRow, 'body' | 'raw_content'>>(
    `SELECT ${summaryColumns} FROM articles ORDER BY published_at DESC NULLS LAST, created_at DESC`,
  );
...
export async function insertArticle(article: NormalizedArticle): Promise<ArticleRecord | null> {
  const result = await query<ArticleRow>(
    ...
    [randomUUID(), article.importKey, article.nzzId, article.documentId, article.headline,
      ...
      article.teaserImage ? JSON.stringify(article.teaserImage) : null, JSON.stringify(article.tags)],
  );
  return result.rows[0] ? mapRow(result.rows[0]) : null;
}
