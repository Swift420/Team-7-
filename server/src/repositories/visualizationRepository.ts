...
const columns =
  "id, article_id, status, placement_after_element_id, specification, created_at, updated_at";

// This cache is only a fallback when PostgreSQL cannot be reached; it is not a durable production store.
...
export async function listArticleVisualizations(
  articleId: string,
...
export async function replaceArticleVisualizations(
  ...
  try {
    return await withTransaction(async (client) => {
      await client.query(
        "DELETE FROM article_visualizations WHERE article_id = $1",
        [articleId],
      );
      ...
      for (const specification of specifications) {
        const result = await client.query<VisualizationRow>(
          ...
          [
            randomUUID(),
            articleId,
            specification.suggestedPlacementAfter,
      ...
      await client.query(
        ...
        [
          ...
          articleId,
          "editor",
          specifications.length ? "saved" : "cleared",
...
export async function listVisualizationApprovals(
  articleId: string,
