...
/** Reads authored q-tool assets without asking Gemini to recreate visuals already in the article. */
const qDataRoot = () =>
  process.env.Q_DATA_DIR ||
  path.resolve(process.cwd(), "../VisualVelocity/input/q_data");
...
function chartData(table: unknown[][], elementId: string) {
  ...
  const series = headers
    .slice(1)
    .map((label, index) => ({ key: `series_${index + 1}`, label }));
  ...
  if (
    data.some(
      (point) =>
        point.values.length !== series.length ||
  ...
  return {
    series,
    data: data.map((point) => ({ ...point, values: point.values as number[] })),
  };
}

export async function loadExistingVisualizations(
  ...
  for (const element of article.body.filter(
    (item) => item.type === "q_tool_embed" && item.externalId,
  )) {
    ...
    const csvFile = files.find(
      (file) => file.startsWith(prefix) && file.endsWith(".csv"),
    );
    const rawTable =
      Array.isArray(payload.data) && payload.data.every(Array.isArray)
        ? (payload.data as unknown[][])
        : csvFile
          ? parseCsv(await fs.readFile(path.join(directory, csvFile), "utf8"))
          : [];
    const { series, data } = chartData(rawTable, element.id);
    const assetUrls = files
      .filter(
        (file) =>
          file.startsWith(prefix) && /\.(png|jpe?g|webp|gif)$/i.test(file),
      )
      .sort()
      .map(
        (file) =>
          `/api/visual-assets/${encodeURIComponent(directoryName)}/${encodeURIComponent(file)}`,
      );
    results.push({
      ...
      chartType:
        payload.tool === "chart"
          ...
          : payload.tool === "choropleth"
            ? "map"
            : payload.tool?.startsWith("election_") && data.length
              ? "bar"
              : null,
      series,
      data,
