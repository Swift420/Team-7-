...
// Builds and optionally synthesizes the short audio derivative used by reader mode.
/**
...
export function buildSSML(
  ...
  clean = clean
    ...
    .replace(
      /\bGKV\b/g,
      '<sub alias="Gesetzliche Krankenversicherung">GKV</sub>',
    )
...
export async function synthesizeAudioBrief(
  ...
  const isGerman = options.language === "de";
  const voiceName =
    options.voiceName || (isGerman ? "de-DE-Studio-B" : "en-US-Journey-F");
  ...
  const isJourneyOrStudio =
    voiceName.includes("Journey") || voiceName.includes("Studio");
  ...
  if (isJourneyOrStudio) {
    ...
    input = {
      ssml: buildSSML(script, options.author, { language: options.language }),
    };
  }
  ...
  if (!token || !projectId) {
    const errorMsg =
      "Google Cloud credentials not found. Cloud TTS is unauthenticated.";
  ...
  console.log(
    `[Google Cloud TTS] Synthesizing speech with voice ${voiceName} (${isJourneyOrStudio ? "plain-text" : "SSML"}) for project ${projectId}...`,
  );
  ...
  if (!data.audioContent) {
    console.error(
      "[Google Cloud TTS] Empty audioContent returned:",
      JSON.stringify(data),
    );
  ...
  console.log(
    `[Google Cloud TTS] Synthesized ${words} words (${durationSeconds}s) successfully`,
  );
  return result;
}
