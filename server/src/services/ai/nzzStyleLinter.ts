...
/** Small deterministic checks provide immediate editor feedback without an AI round trip. */
...
export function lintNZZStyle(
  ...
  if (options.isHeadline) {
    ...
    if (trimmed.includes(":")) {
      ...
      if (parts.length > 1) {
        ...
        if (
          afterColon.length > 0 &&
          afterColon[0] !== afterColon[0].toUpperCase()
        ) {
          errors.push(
            "The first word after a colon in a headline must be capitalized",
          );
        }
      }
    }
  ...
  if (options.isSubhead) {
    ...
    const verbList =
      options.language === "en"
        ? ENGLISH_FINITE_VERBS
        : [...GERMAN_FINITE_VERBS, ...ENGLISH_FINITE_VERBS];

    for (const verb of verbList) {
      if (words.includes(verb)) {
        warnings.push(
          `Subheads should be noun/adjective phrases without finite verbs (found: "${verb}")`,
        );
        break;
      }
    }
  ...
  for (const buzzword of FORBIDDEN_CLICKBAIT_WORDS) {
    if (lower.includes(buzzword)) {
      errors.push(
        `NZZ Voice violation: Avoid sensationalist buzzword "${buzzword}"`,
      );
    }
  }
