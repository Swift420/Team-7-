...
import {
  liquidDerivativesSchema,
  LiquidDerivatives,
  CarouselSlide,
} from "./liquidSchemas.js";
...
/** Orchestrates Gemini generation, normalization, validation, caching, and deterministic fallback output. */
...
export async function generateLiquidDerivatives(
  ...
  const defaultModel = env.geminiModel;
  const model = options.model || defaultModel;
  const language =
    options.language || (article.language as "en" | "de") || "en";
  article.language = language;

  // Explicit demo or mock mode
  if (options.demoMode === true || options.mock === true) {
    console.log(
      "[LiquidEngine] Explicit demo/mock mode active. Generating deterministic output with source=template",
    );
    const generated = generateDeterministicLiquidDerivatives(
      article,
      model,
      language,
    );
  ...
  if (!token || !projectId) {
    const errorMsg =
      "Google Cloud credentials not found. Vertex AI is unauthenticated.";
  ...
  const vertexModel = model.includes("flash")
    ? "gemini-2.5-flash"
    : "gemini-2.5-pro";
  ...
  if (!response.ok) {
    ...
    console.error(
      `[LiquidEngine Vertex AI Error] HTTP ${status}: ${errorBody}`,
    );
  ...
  if (!rawText) {
    console.error(
      "[LiquidEngine Vertex AI] No candidate text returned:",
      JSON.stringify(json),
    );
  ...
  console.log(
    `[LiquidEngine Vertex AI] Successfully synthesized ${vertexModel} derivatives in ${normalized.elapsedMs}ms`,
  );
  return validated;
}
...
export function getContextualTopicImagery(article: ArticleInput): TopicImagery {
  ...
  if (
    ...
    return {
      coverUrl:
        "https://images.unsplash.com/photo-1603048588665-791ca8aea617?auto=format&fit=crop&w=1080&q=80", // Needle on vinyl grooves
      zoomUrl:
  ...
  if (
    /\b(neuro|brain|gehirn|synapse|cortex|cognitive|kognitiv|neural|hirnforschung)\b/i.test(
      text,
    )
  ) {
    return {
      coverUrl:
        "https://images.unsplash.com/photo-1559757175-5700dde675bc?auto=format&fit=crop&w=1080&q=80", // Clinical neuroimaging
      zoomUrl:
  ...
  if (isAutomotive) {
    return {
      coverUrl:
        "https://images.unsplash.com/photo-1614162692292-7ac56d7f7f1e?auto=format&fit=crop&w=1080&q=80", // 911 GT3 high-performance
      zoomUrl:
        "https://images.unsplash.com/photo-1617814076367-b759c7d7e738?auto=format&fit=crop&w=400&q=80", // GT3 cockpit & steering dials
  ...
  if (
    text.includes("u-boot") ||
    text.includes("submarine") ||
    text.includes("maritim") ||
    text.includes("indopazifik") ||
  ...
  const isMilitary =
    text.includes("bundeswehr") ||
    text.includes("truppe") ||
    text.includes("streitkräfte") ||
    text.includes("military") ||
    text.includes("armed forces") ||
    text.includes("rekrut") ||
    text.includes("verteidigung") ||
  ...
  if (isMilitary) {
    return {
      ...
      zoomUrl:
        "https://images.unsplash.com/photo-1579975096649-e773152b04cb?auto=format&fit=crop&w=400&q=80",
  ...
  if (
    ...
    text.includes("wohlfahrt") ||
    text.includes("rente") ||
    text.includes("pension") ||
    text.includes("krankenkasse") ||
    text.includes("sozial") ||
    text.includes("haushalt") ||
...
function normalizeLiquidJson(raw: any, article: ArticleInput): any {
    ...
    for (let i = 0; i < raw.dialecticalFaq.items.length; i++) {
      ...
      if (item.consensusView || item.counterArgument || item.nzzTake) {
        ...
        const ans =
          ...
          item.text ||
          item.summary ||
          `${article.lead || article.headline}. Structural adaptation remains the decisive variable.`;
        unpackedItems.push({
          question: q,
          answer: ans,
          perspective,
  ...
  if (raw.factBox?.metrics && Array.isArray(raw.factBox.metrics)) {
    raw.factBox.metrics = raw.factBox.metrics.map((m: any, i: number) => ({
      id: m.id || `m-${i}`,
      metricName:
        m.metricName || m.name || m.label || "Macroeconomic Indicator",
      value: String(m.value || m.val || "0"),
      delta: m.delta ? String(m.delta) : undefined,
      direction: m.direction || "neutral",
      contextNote:
        m.contextNote || m.note || m.context || "NZZ verified figure",
    }));
  }

  // 4. Normalize scenes prominentMetric
  if (
    raw.socialStoryboard?.scenes &&
    Array.isArray(raw.socialStoryboard.scenes)
  ) {
    raw.socialStoryboard.scenes = raw.socialStoryboard.scenes.map((s: any) => ({
      ...s,
      prominentMetric: s.prominentMetric
        ? String(s.prominentMetric)
        : undefined,
    }));
  }
  ...
  if (raw.visualVelocity?.charts && Array.isArray(raw.visualVelocity.charts)) {
    raw.visualVelocity.charts = raw.visualVelocity.charts
      .map((c: any, i: number) => {
        ...
        return {
          id: c.id || `chart-${i + 1}`,
          chartType: ["line", "bar", "grouped_bar", "area", "scatter"].includes(
            c.chartType,
          )
            ? c.chartType
            : "bar",
          ...
          sourceSentence: String(
            c.sourceSentence || article.lead || article.headline,
          ),
  ...
  const combinedText =
    `${article.headline} ${article.lead} ${article.body || ""}`.toLowerCase();
  const isAutomotive =
    combinedText.includes("porsche") ||
    combinedText.includes("fahrbericht") ||
    combinedText.includes("gt3") ||
    combinedText.includes("sportwagen") ||
    combinedText.includes("sustenpass") ||
    ((combinedText.includes("auto") ||
      combinedText.includes("mobility") ||
      combinedText.includes("mobilität")) &&
      !combinedText.includes("tennis"));

  const isSports =
    !isAutomotive &&
    (combinedText.includes("tennis") ||
      combinedText.includes("grand slam") ||
  ...
  if (!raw.detectedCategory) {
    if (isAutomotive) {
      raw.detectedCategory =
        article.language === "de"
          ? "Mobilität & Automotive"
          : "Mobility & Automotive";
    } else if (isSports) {
      raw.detectedCategory =
        article.language === "de" ? "Sport & Athletik" : "Sports & Athletics";
    } else if (
      combinedText.includes("submarine") ||
      combinedText.includes("u-boot") ||
      ((combinedText.includes("defense") ||
      ...
      combinedText.includes("rente") ||
      combinedText.includes("welfare") ||
      combinedText.includes("sozialstaat")
    ) {
      raw.detectedCategory =
        article.language === "de"
          ? "Wirtschaft & Ordnungspolitik"
          : "Economy & Fiscal Policy";
    } else {
      raw.detectedCategory =
        article.section ||
        (article.language === "de" ? "Wirtschaft" : "Economy");
    }
  }
  ...
  let carouselTheme: "dark" | "sand" | "lavender" | "white" | "grey" =
    raw.instagramCarousel?.theme || "dark";
  let defaultBadge =
    article.language === "de"
  ...
  if (isAutomotive) {
    carouselTheme = "dark";
    defaultBadge =
      article.language === "de" ? "MOBILITÄT & MOTOR" : "MOBILITY & AUTOMOTIVE";
  } else if (isSports) {
    carouselTheme = "sand";
    defaultBadge =
      article.language === "de" ? "SPORT & ATHLETIK" : "SPORTS & ATHLETICS";
  } else if (
    combinedText.includes("klima") ||
    combinedText.includes("climate") ||
    combinedText.includes("temperatur") ||
    combinedText.includes("rente") ||
    combinedText.includes("sozialstaat") ||
    combinedText.includes("inflation") ||
    combinedText.includes("wirtschaft")
  ) {
    carouselTheme = "sand";
    defaultBadge =
      article.language === "de"
    ...
    combinedText.includes("kunst") ||
    combinedText.includes("uhr") ||
    combinedText.includes("craft") ||
    combinedText.includes("kultur") ||
    combinedText.includes("feuilleton")
  ) {
    carouselTheme = "white";
    defaultBadge =
      article.language === "de" ? "KULTUR & HANDWERK" : "CULTURE & CRAFT";
  } else if (
    combinedText.includes("debatte") ||
    combinedText.includes("interview") ||
    combinedText.includes("meinung")
  ) {
    carouselTheme = "lavender";
    defaultBadge =
      article.language === "de" ? "DAS NZZ-INTERVIEW" : "THE NZZ INTERVIEW";
  }
  ...
  if (
    raw.instagramCarousel?.slides &&
    Array.isArray(raw.instagramCarousel.slides)
  ) {
    raw.instagramCarousel.slides = raw.instagramCarousel.slides.map(
      (s: any, idx: number) => {
        ...
        const hasImage =
          s.hasImage !== undefined
            ? Boolean(s.hasImage)
            : isFirst || isSplit || isBalancedVisual;
        ...
        const synthesized = synthesizePhotojournalismPrompt(
          ...
          },
          {
            ...
            category: raw.detectedCategory || article.section,
            lead: article.lead,
          },
        );
        ...
        return {
          ...
          layout:
            ...
            (isFirst
              ? "hook_hero"
              : idx === raw.instagramCarousel.slides.length - 1
                ? "cta_conversion"
                : s.quote
                  ? "quote"
                  : s.metricHighlight
          ...
          metricHighlight: s.metricHighlight?.value
            ? s.metricHighlight
            : undefined,
  ...
  if (!raw.editorialAnalysis) {
    raw.editorialAnalysis = {
      ...
      reasoning:
        "Strict NZZ Inspo design architecture applied across thematic sequence",
    };
  } else {
...
function extractKeyMetrics(text: string): { value: string; label: string }[] {
  ...
  const pctMatches = text.match(
    /(\d+[.,]?\d*)\s*(%|percent|billion|million|euro|dollar|francs|chf|milliarden|prozent)/gi,
  );
  if (pctMatches) {
...
export function generateDeterministicLiquidDerivatives(
  ...
  const headline = (
    article.headline ||
    (isGerman ? "NZZ Hintergrundanalyse" : "NZZ In-Depth Analysis")
  )
    .replace(/\.$/, "")
    .trim();
  const author =
    article.author || (isGerman ? "NZZ Redaktion" : "NZZ Editorial");
  ...
  const quoteMatch = (lead + " " + body).match(
    /«([^»]{15,200})»|"([^"]{15,200})"/,
  );
  ...
  const scenes = [
    ...
    },
    {
      ...
      voiceoverText:
        ...
        allSentences[0] ||
        (isGerman
          ? "Die Ursachen liegen in strukturellen Entwicklungen."
          : "The root causes stem from structural trends."),
    },
    {
      sceneIndex: 4,
      timeRange: "0:35 - 0:48",
      ...
      voiceoverText:
        allSentences[2] ||
        (isGerman
          ? "Die Weichenstellungen treffen auf erhebliche praktische und politische Widerstände."
          : "Policy decisions face considerable operational and institutional resistance."),
    },
    {
      sceneIndex: 5,
      timeRange: "0:48 - 1:00",
  ...
  const isAutomotive =
    textCombined.includes("porsche") ||
    textCombined.includes("fahrbericht") ||
    textCombined.includes("gt3") ||
    textCombined.includes("sportwagen") ||
    textCombined.includes("sustenpass") ||
    ((textCombined.includes("auto") ||
      textCombined.includes("mobility") ||
  ...
  let detectedCategory =
    article.section || (isGerman ? "Wirtschaft" : "Economy");
  let suggestedTags = isGerman
    ? ["#NZZ", "#Analyse", "#Wirtschaft"]
    : ["#NZZ", "#Analysis", "#Policy"];

  if (isAutomotive) {
    detectedCategory = isGerman
      ? "Mobilität & Automotive"
      : "Mobility & Automotive";
    suggestedTags = [
      "#Automotive",
    ...
    ((textCombined.includes("defense") || textCombined.includes("military")) &&
      !textCombined.includes("tennis"))
  ) {
    detectedCategory = isGerman
      ? "Sicherheit & Geopolitik"
    ...
    textCombined.includes("rente") ||
    textCombined.includes("welfare") ||
    textCombined.includes("sozialstaat")
  ) {
    detectedCategory = isGerman
      ? "Wirtschaft & Ordnungspolitik"
      : "Economy & Fiscal Policy";
  ...
  if (isAutomotive) {
    ...
    textCombined.includes("klima") ||
    textCombined.includes("climate") ||
    textCombined.includes("temperatur") ||
    textCombined.includes("rente") ||
    textCombined.includes("sozialstaat") ||
    textCombined.includes("inflation") ||
    textCombined.includes("wirtschaft")
  ) {
    carouselTheme = "sand";
    categoryBadge = isGerman
      ? "DATENANALYSE & WIRTSCHAFT"
      : "DATA ANALYSIS & ECONOMY";
  } else if (
    textCombined.includes("kunst") ||
    textCombined.includes("uhr") ||
    textCombined.includes("craft") ||
    textCombined.includes("kultur") ||
    textCombined.includes("feuilleton")
  ) {
    carouselTheme = "white";
    categoryBadge = isGerman ? "KULTUR & HANDWERK" : "CULTURE & CRAFT";
  } else if (
    textCombined.includes("debatte") ||
    textCombined.includes("interview") ||
    textCombined.includes("meinung")
  ) {
    carouselTheme = "lavender";
    categoryBadge = isGerman ? "DAS NZZ-INTERVIEW" : "THE NZZ INTERVIEW";
  }

  const slide1Meta = synthesizePhotojournalismPrompt(
    { slideNumber: 1, headline, slideType: "cover" },
    { headline, category: detectedCategory, lead },
  );
  const slide2Meta = synthesizePhotojournalismPrompt(
    {
      slideNumber: 2,
      headline: isGerman ? "Kontext & Daten" : "The Defining Metric",
      slideType: "data_point",
    },
    { headline, category: detectedCategory, lead },
  );
  const slide3Meta = synthesizePhotojournalismPrompt(
    {
      slideNumber: 3,
      headline: isGerman ? "Der Kausalzusammenhang" : "The Structural Context",
      slideType: "context",
    },
    { headline, category: detectedCategory, lead },
  );
  const slide4Meta = synthesizePhotojournalismPrompt(
    {
      slideNumber: 4,
      headline: isGerman ? "Die Bruchlinien" : "The Strategic Dichotomy",
      slideType: "context",
    },
    { headline, category: detectedCategory, lead },
  );
  const slide5Meta = synthesizePhotojournalismPrompt(
    {
      slideNumber: 5,
      headline: isGerman ? "Drei Treiber" : "Strategic Horizons",
      slideType: "consequences",
    },
    { headline, category: detectedCategory, lead },
  );
  const slide6Meta = synthesizePhotojournalismPrompt(
    {
      slideNumber: 6,
  ...
  const slide7Meta = synthesizePhotojournalismPrompt(
    {
      slideNumber: 7,
      headline: isGerman ? "NZZ Fazit" : "In-Depth Analysis at NZZ",
      slideType: "outro",
    },
    { headline, category: detectedCategory, lead },
  );
  ...
  const carouselSlides: CarouselSlide[] = [
    ...
    },
    {
      ...
      chartData: {
        ...
        items: [
          {
            label: isGerman ? "Referenz 2018-2022" : "Baseline 2018-2022",
            value: "18.5%",
            isHighlighted: false,
          },
          {
            label: isGerman ? "Durchschnitt 2023-2024" : "Average 2023-2024",
            value: "21.2%",
            isHighlighted: false,
          },
          {
            label: isGerman ? "Aktueller Messwert" : "Current Measure",
            value: primaryMetric.value || "24.9%",
            isHighlighted: true,
          },
        ],
        caption: isGerman
          ? "Quelle: NZZ Datenrecherche"
          : "Source: NZZ Quantitative Analysis",
      },
      comparisonCards: {
        card1: {
    ...
    },
    {
      ...
      hasImage: false,
      headline: isGerman
        ? "Die strategische Bruchlinie"
        : "The Strategic Dichotomy",
      comparisonCards: {
        card1: {
          title: isGerman
            ? "PERSPEKTIVE A: VORTEIL"
            : "PERSPECTIVE A: ADVANTAGE",
        ...
        card2: {
          title: isGerman
            ? "PERSPEKTIVE B: RISIKO"
            : "PERSPECTIVE B: VULNERABILITY",
      ...
      bulletItems: [
        {
          ...
          text: bullet1.slice(0, 75),
        },
        {
          icon: "energy",
          title: isGerman ? "Dynamik" : "Dynamics",
          text: bullet2.slice(0, 75),
        },
        {
          icon: "alert",
          title: isGerman ? "Konsequenz" : "Consequence",
          text: bullet3.slice(0, 75),
        },
      ],
  ...
  const factMetrics = [
    {
      ...
      contextNote: isGerman
        ? "Aus dem Artikeltext verifizierter Wert"
        : "Verified figure from article source",
    },
    {
      id: "m-2",
      ...
      direction: "neutral" as const,
      contextNote: isGerman
        ? "Aktueller Berichts- und Analysezeitraum"
        : "Current reporting and analysis baseline",
    },
    {
      id: "m-3",
      ...
      contextNote: isGerman
        ? "Einschätzung gemäss NZZ-Fachredaktion"
        : "Assessed by NZZ editorial desk",
    },
  ];
  ...
  const wordCount =
    (article as any).wordCount ||
    (body || lead).split(/\s+/).filter(Boolean).length;
  let depth: "brief" | "standard" | "deep" = "deep";
  let slideCount = 7;
  if (
    (article as any).depth === "brief" ||
    ((article as any).wordCount && (article as any).wordCount < 300)
  ) {
    depth = "brief";
    slideCount = 4;
  ...
  const effectiveSlides = carouselSlides
    .slice(0, slideCount)
