...
export async function importArticleDirectory(directory: string): Promise<DatasetImportReport> {
  ...
  for (const entry of entries) {
    try {
      const outcome = await importArticleContent(entry.name, await fs.readFile(path.join(directory, entry.name)));
      report[outcome.status].push(entry.name);
      console.log(`${outcome.status}: ${entry.name} (${outcome.article.headline})`);
    } catch (error) {
