...
// These parsers are the trust boundary for imported newsroom data; malformed input never reaches persistence.
const optionalText = z.union([z.string(), z.number()]).nullish();
const nzzArticleSchema = z
  .object({
    nzz_id: optionalText,
    ...
    body: z
      ...
      .min(1, "body must contain at least one element"),
...
const documentIdOrNull = (value: unknown): string | null => {
  ...
  if (documentId && !/^\d+$/.test(documentId)) {
    throw new ArticleValidationError("Invalid document_id", [
      "document_id must contain digits only",
    ]);
  }
  return documentId;
};
...
const elementId = (index: number) =>
  `element-${String(index + 1).padStart(4, "0")}`;

function normalizeNzzElement(
  ...
  if (rawType === "image") {
    const image = (
      element.image && typeof element.image === "object"
        ? element.image
        : element
  ...
  if (rawType === "embed") {
    ...
    return {
      ...base,
      type: service?.toLowerCase().includes("q-tool")
        ? "q_tool_embed"
        : "embed",
      service,
...
export function parseNzzJson(rawText: string): NormalizedArticle {
  ...
  try {
    ...
    throw new ArticleValidationError("Invalid JSON file", [
  ...
  if (
    !body.some(
      (element) => element.type === "paragraph" && element.text?.trim(),
    )
  ) {
    throw new ArticleValidationError("Invalid NZZ article JSON", [
      "body must contain article text",
    ]);
  }
  return {
...
function parseFrontMatter(text: string): {
  ...
  if (!match)
    throw new ArticleValidationError("Invalid Markdown front matter", [
...
export function parseMarkdown(rawText: string): NormalizedArticle {
  if (!rawText.trim())
    throw new ArticleValidationError("Invalid Markdown article", [
      "Article text is required",
    ]);
  const { metadata, content: withHeader } = parseFrontMatter(
    rawText.replace(/^\uFEFF/, ""),
  );
  const lines = withHeader.split(/\r?\n/);
  const headlineIndex = lines.findIndex((line) => /^#\s+/.test(line));
  const headline =
    metadataValue(metadata, "headline", "title") ||
    (headlineIndex >= 0
      ? lines[headlineIndex].replace(/^#\s+/, "").trim()
      : null);
  if (!headline) {
    throw new ArticleValidationError("Invalid Markdown article", [
      "A level-one heading (# Headline) or title front-matter field is required",
    ]);
  }
  ...
  let publishedText = metadataValue(
    metadata,
    "published_at",
    "date",
    "publication_date",
  ...
  if (!bodyText)
    throw new ArticleValidationError("Invalid Markdown article", [
      "Article body text is required",
    ]);
  const body = markdownBodyElements(bodyText);
  const hasArticleText = body.some(
    (element) => element.type === "paragraph" && element.text?.trim(),
  );
  if (!hasArticleText)
    throw new ArticleValidationError("Invalid Markdown article", [
      "Article body must contain text",
    ]);
...
export function parseArticle(
  ...
  return sourceFormat === "NZZ_JSON"
    ? parseNzzJson(rawText)
