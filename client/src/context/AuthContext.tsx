...
function getStoredAuth(): { user: User; isAuthenticated: boolean; token: string | null } {
  if (typeof window === 'undefined') return { user: GUEST_USER, isAuthenticated: false, token: null };
  ...
  try {
    ...
    if (saved?.role === 'editor') return { user: saved, isAuthenticated: true, token };
  } catch {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const logout = useCallback(() => { localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(STORAGE_KEY); setApiAuthToken(null); setCurrentUser(GUEST_USER); setIsAuthenticated(false); }, []);
  const switchRole = useCallback(() => { if (isAuthenticated) logout(); }, [isAuthenticated, logout]);
  const contextValue = useMemo(() => ({ currentUser, isEditor: isAuthenticated, isViewer: !isAuthenticated, isAuthenticated, login, logout, switchRole }), [currentUser, isAuthenticated, login, logout, switchRole]);
