...
export const VisualizationChart: React.FC<VisualizationChartProps> = ({
  opportunity,
  chartType = opportunity.chartType,
}) => {
  const data = opportunity.data.map((point) => {
    const row: Record<string, string | number> = { label: point.label };
    opportunity.series.forEach((series, index) => {
      row[series.key] = point.values[index];
    });
    return row;
  });
  const shared = {
  ...
  const axes = (
    <>
      ...
      <Tooltip
        ...
        formatter={(value) => [
          `${String(value)}${opportunity.unit ? ` ${opportunity.unit}` : ""}`,
        ]}
      />
  ...
  if (chartType === "donut") {
    const pieData = opportunity.data.map((point) => ({
      name: point.label,
    ...
    return (
      <div
        ...
        role="img"
        aria-label={opportunity.accessibilitySummary}
      >
    ...
    return (
      ...
      >
        {(opportunity.events || []).map((event, index) => (
          <div className="timeline-event" key={`${event.dateLabel}-${index}`}>
            <div className="timeline-marker" />
            <button
              className="timeline-event-content"
              type="button"
              onClick={() => {
                const source = event.sourceParagraphIds
            ...
            >
              ...
              <small>View source context</small>
            </button>
          </div>
        ))}
      </div>
          ...
          <ScatterChart margin={{ top: 12, right: 20, left: 36, bottom: 8 }}>
            ...
            {opportunity.series.map((series, seriesIndex) => (
              <Scatter
                ...
                data={opportunity.data.map((point, row) => ({
                  row,
                  label: point.label,
                  value: point.values[seriesIndex],
                }))}
              />
            ))}
            {opportunity.series.length > 1 && <Legend />}
    ...
    >
      <ResponsiveContainer width="100%" height="100%">
        {chartType === "line" ? (
          <LineChart {...shared}>
            {axes}
            {opportunity.series.map((series, index) => (
              <Line
                key={series.key}
                type="monotone"
                dataKey={series.key}
