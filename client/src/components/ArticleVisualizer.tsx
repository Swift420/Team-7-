...
import {
  analyzeArticleVisualizations,
  saveArticleVisualizations,
} from "../services/api";
import {
  Article,
  SavedVisualization,
...
// Analysis is editor-triggered; saved approvals load separately so drafts are never auto-published.
...
export const ArticleVisualizer: React.FC<ArticleVisualizerProps> = ({
  ...
  const [chartTypes, setChartTypes] = useState<
    Record<string, VisualizationChartType>
  >({});
  ...
  useEffect(() => {
    if (
      new URLSearchParams(window.location.search).get("visualize") === "1" &&
  ...
  const updateOpportunity = (
    id: string,
    update: (opportunity: VisualizationOpportunity) => VisualizationOpportunity,
  ) => {
    setAnalysis((current) =>
      current
        ? {
  ...
  const saveSelected = async () => {
    ...
    try {
      const selected = analysis.opportunities
        ...
        .map((opportunity) => ({
          ...opportunity,
          chartType: chartTypes[opportunity.id] || opportunity.chartType,
        }));
      const saved = await saveArticleVisualizations(article.id, selected);
      onSaved(saved);
      setSaveMessage(
        `${selected.length} ${selected.length === 1 ? "visual" : "visuals"} added to the article.`,
      );
    } catch (cause) {
      setError(
        cause instanceof Error
  ...
  const clearSaved = async () => {
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Unable to remove saved visualizations",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section
      className="article-visualizer"
      aria-labelledby="article-visualizer-title"
    >
      <div className="visualizer-intro">
        ...
        <div className="visualizer-copy">
          ...
          <p>
            Find trustworthy comparisons and trends not already covered by
            existing visuals, edit their data, then save them inline.
          </p>
          {(savedVisualizations.length > 0 || approvalCount > 0) && (
            <div className="visual-status-row">
              {savedVisualizations.length > 0 && (
                <span className="saved-visual-count">
                  <Check size={12} /> {savedVisualizations.length} visual
                  {savedVisualizations.length === 1 ? "" : "s"} saved{" "}
                  <button
              ...
              {approvalCount > 0 && (
                <span className="approval-count-badge">
                  {approvalCount} approval snapshot
                  {approvalCount === 1 ? "" : "s"}
                </span>
              )}
            </div>
        ...
        <div className="visualizer-intro-actions">
          <a
            ...
            href={`/api/articles/${article.id}/visualizations/embed`}
            target="_blank"
            rel="noreferrer"
          >
            Export embed
      ...
      {analysis && (
        <div className="visualizer-results">
          <div className="analysis-summary">
            <div>
              <strong>
                {analysis.opportunities.length} opportunities found
              </strong>
          ...
          {analysis.opportunities.length === 0 ? (
            <div className="no-visual-opportunities">
              Gemini did not find enough explicit, comparable data to build a
              trustworthy chart from this article alone.
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="opportunity-title-row">
                      <div className="visual-copy-editor">
                        ...
                        />
                        <input
                          className="visual-subtitle-input"
                          aria-label="Visualization subtitle"
                          value={opportunity.subtitle}
                    ...
                    <p className="opportunity-rationale">
                    ...
                    <div className="chart-controls">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setChartTypes((current) => ({
                              ...current,
                              [opportunity.id]: event.target
                                .value as VisualizationChartType,
                            }))
                          }
                        >
                      ...
                      <span>
                        {opportunity.data.length} data points ·{" "}
                        {opportunity.series.length}{" "}
                    ...
                    />
                    <p className="visual-plain-summary">
                      What this shows:{" "}
                    ...
                    <div className="visual-data-editor">
                      <div className="data-editor-heading">
                        <strong>Editable chart data</strong>
                        <span>Changes update the preview immediately</span>
                      </div>
                      <div className="data-table-scroll">
                        <table>
                          <thead>
                            <tr>
                              <th>Label</th>
                              {opportunity.series.map((series, seriesIndex) => (
                                <th key={series.key}>
                                  <input
                          ...
                          <tbody>
                            {opportunity.data.map((point, pointIndex) => (
                              <tr key={`${point.label}-${pointIndex}`}>
                                <td>
                                  <input
                                    aria-label={`Data row ${pointIndex + 1} label`}
                                    value={point.label}
                                    onChange={(event) =>
                                      updateOpportunity(
                                        opportunity.id,
                                ...
                                {point.values.map((value, valueIndex) => (
                                  <td key={valueIndex}>
                                    <input
                                      type="number"
                                      aria-label={`Value ${pointIndex + 1}, ${valueIndex + 1}`}
                                      value={value}
                                      onChange={(event) => {
                    ...
                    <div className="opportunity-footer">
                      <div className="source-links">
                        <LocateFixed size={14} /> Evidence:{" "}
                        {sourceIds.slice(0, 4).map((id) => (
                          <button key={id} onClick={() => jumpToSource(id)}>
                            {id}
                          </button>
                        ))}
                      </div>
                      {opportunity.caveats.length > 0 && (
                        <p className="visual-caveat">
                          Review: {opportunity.caveats.join(" · ")}
                        </p>
                      )}
                    </div>
                  </article>
          ...
          {analysis.opportunities.length > 0 && (
            <div className="visualizer-actions">
              ...
              <button
                className="insert-visuals-button"
              ...
              >
                ...
                {saving ? "Saving…" : "Save selected inline"}
              </button>
              {saveMessage && (
                <span
                  className="visual-save-confirmation"
                  role="status"
                  aria-live="polite"
                >
