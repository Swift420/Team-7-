...
type GlobeArc = CountryConnection & {
  startLat: number;
  startLng: number;
  endLat: number;
  endLng: number;
...
// Coverage is lightweight metadata; full story summaries load only after a country is selected.
...
export const StoryGlobeExplorer: React.FC<{ onClose?: () => void }> = ({
  onClose,
}) => {
  ...
  const [selectedConnection, setSelectedConnection] =
    useState<CountryConnection | null>(null);
  ...
  const coverageByCode = useMemo(
    () => new Map(coverage.map((item) => [item.countryCode, item])),
    [coverage],
  );
  const coverageForFeature = (item: GlobeFeature | null) => {
    ...
    const code =
      ...
      countryAliases[name] ||
      coverage.find(
        (entry) => entry.countryName.toLowerCase() === name.toLowerCase(),
      )?.countryCode;
    return code ? coverageByCode.get(code) : undefined;
  };
  const polygons = useMemo(
    () =>
      topologyFeatures.features.map((item) => {
        const name = item.properties?.name || "";
        const code =
          countryAliases[name] ||
          coverage.find(
        ...
        return {
          ...item,
          countryCode: code,
          coverage: code ? coverageByCode.get(code) : undefined,
        };
      }),
    [coverage, coverageByCode],
  );
  const totalStories = coverage.reduce(
    (total, item) => total + item.storyCount,
    0,
  );
  ...
  const arcs = useMemo<GlobeArc[]>(
    () =>
      connections.flatMap((connection) => {
        const start = polygonCenters.get(
          connection.source.countryCode.toUpperCase(),
        );
        const end = polygonCenters.get(
          connection.target.countryCode.toUpperCase(),
        );
        return start && end
          ? [
              {
  ...
  const visibleArcs = useMemo(() => {
    const activeCode =
      ...
      selectedConnection?.source.countryCode ||
      selectedConnection?.target.countryCode;
    if (!showConnections || !activeCode) return [];
    return arcs.filter(
      (arc) =>
        arc.source.countryCode === activeCode ||
        arc.target.countryCode === activeCode,
    );
  ...
  const selectCountry = async (item: GlobeFeature) => {
    ...
    if (center)
      globeRef.current?.pointOfView({ ...center, altitude: 1.65 }, 700);
    try {
      setStories(await fetchCountryStories(itemCoverage.countryCode));
    } finally {
  ...
  return (
    <section
      className="story-globe-explorer"
      aria-labelledby="story-globe-title"
    >
      <div className="story-globe-heading">
        <div>
          <span className="section-type-pill">
            <MapPin size={13} /> Global story explorer
          </span>
          <h2 id="story-globe-title">Explore stories around the world</h2>
          <p>
            Countries with coverage are highlighted. Rotate the globe or select
            a country to discover related reporting.
          </p>
        </div>
        <div className="story-globe-heading-actions">
          ...
          {onClose && (
            <button
              ...
              onClick={onClose}
              aria-label="Close global story explorer"
            >
              <X size={17} />
            </button>
          )}
      ...
      <div className="story-globe-layout">
        <div ref={containerRef} className="story-globe-canvas">
          <Globe
            ref={globeRef}
            width={width}
            height={Math.min(600, Math.max(360, width * 0.66))}
            ...
            polygonGeoJsonGeometry={(item) =>
              (item as GlobeFeature).geometry as never
            }
            polygonCapColor={() => "rgba(0,0,0,0)"}
            polygonSideColor={() => "rgba(0,0,0,0)"}
            polygonStrokeColor={(item) =>
              coverageForFeature(item as GlobeFeature)?.storyCount
                ? "#ef4444"
                : "#000000"
            }
            ...
            polygonLabel={(item) => {
              ...
              const name =
                itemCoverage?.countryName ||
                value.properties?.name ||
                "Country";
              const count = itemCoverage?.storyCount || 0;
              return `<div class="globe-tooltip"><strong>${name}</strong><span style="display:block;margin-top:3px">${count} ${count === 1 ? "Story" : "Stories"}</span></div>`;
            }}
            onPolygonHover={(item) =>
              setHovered(
                coverageForFeature(item as GlobeFeature | null) || null,
              )
            }
            ...
            arcColor={(arc: object) =>
              (arc as GlobeArc).storyCount > 1
                ? "rgba(251,146,60,.78)"
                : "rgba(251,146,60,.48)"
            }
            arcStroke={(arc) =>
              (arc as GlobeArc).storyCount > 1 ? 0.45 : 0.22
            }
            arcAltitudeAutoScale={0.28}
            arcLabel={(arc) => {
              const value = arc as GlobeArc;
              return `<div class="globe-tooltip"><strong>${value.source.countryName} ↔ ${value.target.countryName}</strong><span style="display:block;margin-top:3px">${value.storyCount} shared ${value.storyCount === 1 ? "story" : "stories"}</span></div>`;
            }}
            onArcClick={(arc) => {
              const value = arc as GlobeArc;
              setSelected(null);
          ...
          {hovered && (
            <div className="story-globe-hover-card">
              ...
              <span>
                {hovered.storyCount}{" "}
                {hovered.storyCount === 1 ? "story" : "stories"}
              </span>
            </div>
          )}
        </div>
        <aside
          className={`story-country-panel ${selected ? "is-selected" : ""}`}
        >
          {selectedConnection ? (
            <>
              <div className="story-country-panel-header">
                <div>
                  <span className="story-panel-kicker">Shared coverage</span>
                  <h3>
                    {selectedConnection.source.countryName} ↔{" "}
                    {selectedConnection.target.countryName}
                  </h3>
                  <p>
                    {selectedConnection.storyCount} shared{" "}
                    {selectedConnection.storyCount === 1 ? "story" : "stories"}
                  </p>
                </div>
                <button
                  className="story-panel-close"
                  onClick={resetGlobeView}
                  aria-label="Close shared stories"
                >
                  <X size={17} />
                </button>
              </div>
              <div className="country-story-list">
                {selectedConnection.stories.map((story) => (
                  ...
                  >
                    ...
                    <span>
                      {story.section || "Uncategorised"} ·{" "}
                      {formatDate(story.publishedAt)}
                    </span>
                    {story.lead && <p>{story.lead}</p>}
                    <small>Open story →</small>
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              <div className="story-country-panel-header">
                <div>
                  <span className="story-panel-kicker">Selected country</span>
                  <h3>{selected.countryName}</h3>
                  <p>
                    {selected.storyCount} related{" "}
                ...
                <button
                  className="story-panel-close"
                  onClick={resetGlobeView}
                  aria-label="Close country stories"
                >
                  <X size={17} />
                </button>
              </div>
              {loadingStories ? (
                <div className="story-panel-loading">
                  <LoaderCircle className="spin" size={19} /> Loading stories…
                </div>
              ) : stories.length ? (
                <div className="country-story-list">
                  {stories.map((story) => (
                ...
                <p className="story-panel-empty">
                  No stories currently available for this country.
                </p>
              )}
            </>
          ) : (
            <>
              ...
              <p className="story-panel-hint">
                Select a highlighted country to browse its stories.
              </p>
            </>
          )}
        </aside>
