...
export const StoryGlobeExplorer: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
  ...
  return <section className="story-globe-explorer" aria-labelledby="story-globe-title">
    <div className="story-globe-heading"><div><span className="section-type-pill"><MapPin size={13} /> Global story explorer</span><h2 id="story-globe-title">Explore stories around the world</h2><p>Countries with coverage are highlighted. Rotate the globe or select a country to discover related reporting.</p></div><div className="story-globe-heading-actions"><div className="story-globe-stats"><strong>{coverage.length}</strong><span>countries</span><strong>{totalStories}</strong><span>stories</span></div>{onClose && <button className="story-panel-close story-globe-close" onClick={onClose} aria-label="Close global story explorer"><X size={17} /></button>}</div></div>
