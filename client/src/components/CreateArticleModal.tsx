...
export const CreateArticleModal: React.FC<CreateArticleModalProps> = ({ isOpen, mode, onClose }) => {
  ...
  const submitCreate = async (visualize: boolean) => {
    ...
    try {
      ...
      onClose(); await openArticle(outcome.article.id);
      if (visualize) { window.history.replaceState({}, '', `/articles/${outcome.article.id}?visualize=1`); window.dispatchEvent(new PopStateEvent('popstate')); }
