...
export const CreateArticleModal: React.FC<CreateArticleModalProps> = ({
  ...
  const handleImageUrlChange = (inputUrl: string) => {
    let url = inputUrl.trim();
    // Auto-convert Unsplash photo page URLs (e.g. https://unsplash.com/photos/person-holding-circular-light-kTJ8MnqQozg or https://unsplash.com/photos/kTJ8MnqQozg)
    // to direct download image URL which redirects cleanly to CDN photo JPG
    const unsplashPageMatch = url.match(
      /^https?:\/\/(?:www\.)?unsplash\.com\/photos\/(?:[a-zA-Z0-9_-]+-)?([a-zA-Z0-9_-]+)\/?$/i
    );
    if (unsplashPageMatch && !url.includes('images.unsplash.com')) {
      const photoId = unsplashPageMatch[1];
      url = `https://unsplash.com/photos/${photoId}/download?w=1200`;
    }
    setImageUrl(url);
    setImagePreview(url);
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        ...
        {mode === 'import' ? (
          <form onSubmit={submitImport} className="nzz-editor-scroll flex flex-col justify-between">
            <div>
            ...
            <div className="nzz-dialog-footer border-t border-zinc-200 -mx-7 -mb-6 mt-6 px-7 py-3.5 bg-zinc-50">
              <div className="nzz-dialog-actions flex items-center justify-end gap-2 m-0">
          ...
          <>
            <div className="nzz-editor-scroll">
              ...
              <div className="nzz-image-upload-section">
                ...
                {imagePreview ? (
                  <div className="nzz-image-preview-wrap">
                    {imageLoadError ? (
                      <div className="w-full h-full flex flex-col items-center justify-center p-4 text-center bg-zinc-100 text-zinc-500">
                        <Camera size={24} className="mb-1 text-zinc-400" />
                        <span className="text-xs font-semibold text-zinc-700">
                          {isEn ? 'Preview unavailable' : 'Vorschau nicht verfügbar'}
                        </span>
                        <span className="text-[11px] text-zinc-500 mt-0.5">
                          {isEn
                            ? 'Please provide a direct image link (e.g. ending in .jpg) or upload a file directly.'
                            : 'Bitte direkten Bildlink verwenden oder Datei direkt hochladen.'}
                        </span>
                      </div>
                    ) : (
                      <img
                        ...
                        className="nzz-image-preview-img"
            ...
            <div className="nzz-dialog-footer">
              <div className="nzz-dialog-actions flex flex-wrap items-center justify-between gap-2 w-full m-0">
                <div className="flex items-center gap-2">
                  <button type="button" className="nzz-btn-cancel" onClick={onClose}>
                    {isEn ? 'Cancel' : 'Abbrechen'}
                  </button>
