  ...
  return <div className="article-feed-container">
    <div className="section-header-banner">
      ...
      <div className="header-action-group">
        ...
        {isEditor && <button className="btn-feed-create" onClick={() => setIsCreateModalOpen(true)}><Plus size={16} /> Import Article</button>}
    ...
    {loading ? <div className="empty-state-box"><RefreshCw className="spin" /><p>Loading articles from PostgreSQL…</p></div>
      ...
      : <div className="empty-state-box"><FileText size={48} className="empty-icon" /><h3>No articles found</h3><p>{query ? `No imported articles matched “${searchQuery}”.` : 'Import the supplied JSON dataset or upload an article to begin.'}</p>{isEditor && <button className="btn-create-empty" onClick={() => setIsCreateModalOpen(true)}><Plus size={16} /> Import Article</button>}</div>}
  </div>;
};
