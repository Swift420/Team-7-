...
import { BookOpen, ChevronDown, FilePenLine, FileUp, Lock, Search, Shield, UserRound } from 'lucide-react';
  ...
  return <header className="navbar-container">
    <div className="navbar-top">
      ...
      <div className="navbar-top-actions">
        ...
        <button className="btn-quick-role-toggle" onClick={() => isEditor ? logout() : setIsAuthModalOpen(true)}><span className="toggle-label">{isEditor ? 'Session:' : 'Editor access:'}</span><span className={`toggle-pill ${isEditor ? 'to-viewer' : 'to-editor'}`}>{isEditor ? 'Sign out' : 'Sign in'}</span></button>
        <div className="user-profile-menu-container"><button className="user-profile-btn" onClick={() => setIsAuthModalOpen(true)}>{isEditor ? <img src={currentUser.avatar} alt="" className="user-avatar-top" /> : <span className="user-avatar-top user-avatar-icon"><UserRound size={18} /></span>}<div className="user-info-text"><span className="user-display-name">{isEditor ? currentUser.name : 'Reader'}</span>{isEditor && <span className={`user-role-badge badge-${currentUser.role}`}><Shield size={10} />EDITOR</span>}</div><ChevronDown size={14} /></button></div>
      </div>
    </div>
