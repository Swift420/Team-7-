...
import { BookOpen, ChevronDown, FileUp, Lock, Search, Shield, UserCheck } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useArticles } from '../context/ArticleContext';

export const Navbar: React.FC = () => {
  const { currentUser, isEditor, switchRole } = useAuth();
  const { selectedCategory, setSelectedCategory, setIsCreateModalOpen, setIsAuthModalOpen, searchQuery, setSearchQuery, articles } = useArticles();
  const sections = [...new Set(articles.map((article) => article.section).filter((value): value is string => Boolean(value)))].sort();

  const handleImport = () => isEditor ? setIsCreateModalOpen(true) : setIsAuthModalOpen(true);
  ...
  return <header className="navbar-container">
    ...
    <nav className="navbar-sections-nav">
      <div className="nav-tabs-scroll">
        <button className={`section-nav-tab ${selectedCategory === 'all' ? 'active' : ''}`} onClick={() => setSelectedCategory('all')}>All Articles</button>
        {sections.map((section) => <button key={section} className={`section-nav-tab ${selectedCategory === section ? 'active' : ''}`} onClick={() => setSelectedCategory(section)}>{section}</button>)}
      </div>
