...
export function summarizeVisualization(visual: VisualizationOpportunity): string {
  ...
  if (!visual.data.length || !visual.series.length) return 'This visual has no numeric data to summarize yet.';
  const first = visual.data[0];
  const last = visual.data[visual.data.length - 1];
  const primary = visual.series[0].label;
  const highest = visual.data.reduce((best, point) => point.values[0] > best.values[0] ? point : best, first);
  const lowest = visual.data.reduce((best, point) => point.values[0] < best.values[0] ? point : best, first);
  const direction = last.values[0] > first.values[0] ? 'increases' : last.values[0] < first.values[0] ? 'decreases' : 'stays broadly level';
