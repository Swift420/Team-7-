...
// This screen coordinates generation; each artifact preview owns its editing/export behavior.
...
export const ArticleStudio: React.FC<ArticleStudioProps> = ({
  ...
  const [generatingFormatName, setGeneratingFormatName] =
    useState<string>("Instagram Carousel");
  const [activeArtifact, setActiveArtifact] =
    useState<ActiveArtifactType>(null);
  ...
  const handleGenerateArtifact = async (
    artifactType: "carousel" | "audio" | "video" | "newsletter",
  ) => {
    const formatLabels: Record<string, string> = {
      carousel:
        language === "de"
          ? "Instagram Karussell (Stil A)"
          : "Instagram Carousel (Style A)",
      audio: language === "de" ? "Audio-Briefing (60s)" : "Audio Brief (60s)",
      video:
        language === "de"
          ? "Vertikales Video-Storyboard"
          : "Vertical Video Storyboard",
      newsletter:
        language === "de"
          ? "Executive Brief & Fact Box"
          : "Executive Brief & Fact Box",
    };
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-4 text-xs text-stone-400">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>
              {Math.round((article.wordCount || 1200) / 200)} min read (
              ...
              <span>
                {showFullBody
                  ? language === "de"
                    ...
                    : "Hide full text"
                  : language === "de"
                    ? "Vollständigen Artikeltext anzeigen"
                    : "Read source article text"}
              </span>
              {showFullBody ? (
        ...
        {genError && (
          <div className="mb-4 p-4 rounded-xl bg-red-950/80 border border-red-800 text-sm text-red-200">
            <div className="font-semibold text-red-400 mb-1">
              {language === "de"
                ? "Fehler bei der Modellgenerierung:"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          >
            ...
            <div>
              ...
              <div className="text-[11px] text-stone-400 mt-0.5 font-serif">
                {language === "de"
                  ? "Stil A (Schweizer Prestige, 6 Folien)"
                  : "Style A (Swiss Prestige, 6 Slides)"}
              </div>
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              <div className="text-[11px] text-stone-400 mt-0.5 font-serif">
                {language === "de"
                  ? "Natürliche Sprachausgabe (60s)"
                  : "140 Words, Studio Cadence"}
              </div>
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              <div className="text-[11px] text-stone-400 mt-0.5 font-serif">
                {language === "de"
                  ? "5 Szenen für TikTok/Reels"
                  : "5-Scene Storyboard for Reels"}
              </div>
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              <div className="text-[11px] text-stone-400 mt-0.5 font-serif">
                {language === "de"
                  ? "3 Thesen & Faktenleiste"
                  : "3 Key Bullets & Fact Metrics"}
              </div>
            </div>
          </button>
        </div>
          ...
          <div className="flex items-center justify-between pb-6 mb-6 border-b border-stone-800">
            <div>
              <div className="text-xs uppercase font-mono tracking-widest text-red-500">
                {language === "de"
                  ? "Generiertes Medien-Artefakt"
                  : "Generated Media Artifact"}
              </div>
              <h3 className="text-xl font-serif font-bold text-white mt-1">
                {activeArtifact === "carousel" &&
                  (language === "de"
                    ? "Instagram Karussell (Stil A - Schweizer Prestige)"
                    : "Instagram Carousel (Style A - Swiss Prestige)")}
                {activeArtifact === "audio" &&
                  (language === "de"
                    ? "60-Sekunden Audio-Briefing"
                    : "60-Second Audio Brief")}
                {activeArtifact === "video" &&
                  (language === "de"
                    ? "Vertikales Video-Storyboard (9:16)"
                    : "Vertical Video Storyboard (9:16)")}
                {activeArtifact === "newsletter" &&
                  (language === "de"
                    ? "Executive Brief & Faktenleiste"
                    : "Executive Newsletter & Fact Box")}
              </h3>
            </div>
            ...
            <div className="max-w-xl mx-auto space-y-6">
              ...
              <div className="p-4 bg-stone-900 border border-stone-800 rounded-xl">
                <div className="text-xs font-semibold text-stone-400 uppercase tracking-wider mb-2">
                  {language === "de"
                    ? "Gesprochenes Manuskript (140 Wörter)"
                    : "Spoken Script (140 Words)"}
                </div>
              ...
              {derivatives.executiveNewsletter && (
                <div className="p-6 bg-stone-900 border border-stone-800 rounded-xl space-y-4">
                  ...
                  <ul className="space-y-3">
                    {derivatives.executiveNewsletter.bullets.map(
                      (bullet, i) => (
                        <li
                          key={i}
                          className="flex items-start gap-3 text-stone-300 text-sm font-serif"
                        >
              ...
              {derivatives.factBox?.metrics && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {derivatives.factBox.metrics.map((m) => (
                    <div
                      key={m.id}
                      className="p-4 bg-stone-900 border border-stone-800 rounded-xl"
                    >
                      <div className="text-2xl font-serif font-black text-white">
                        {m.value}
                      </div>
                      <div className="text-xs uppercase font-semibold text-stone-400 mt-1">
                        {m.metricName}
                      </div>
                      {m.contextNote && (
