...
// Draft generation is separate from article persistence; the editor chooses when to save.
const PRESETS = [
  {
    id: "lockbit-de",
...
Ordnungspolitisch steht die Schweiz vor einer Richtungsentscheidung: Soll eine gesetzliche Meldepflicht für Ransomware-Vorfälle und ein striktes Lösegeld-Zahlungsverbot nach französischem Vorbild eingeführt werden? Während Sicherheitsbehörden ein Zahlungsverbot befürworten, um das kriminelle Geschäftsmodell auszutrocknen, warnen Wirtschaftsverbände vor existenzbedrohenden Konsequenzen für betroffene Firmen.`,
  },
  {
    ...
    author: "Dr. Beat Gygi",
    headline:
      "European Defense Spending Surges Past €380 Billion Amid NATO Strategic Realignment",
...
export const DraftStudio: React.FC<DraftStudioProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "carousel" | "audio" | "video" | "newsletter" | "charts" | "faq"
  >("carousel");
  ...
  const [derivatives, setDerivatives] =
    useState<LiquidDerivativesPayload | null>(null);
  ...
  const handleGenerate = async (focusTab?: typeof activeTab) => {
    ...
    if (!body.trim() && !lead.trim()) {
      setErrorMessage(
        lang === "de"
          ? "Bitte fügen Sie den Artikeltext in das Textfeld ein."
          : "Please paste or write your article body text.",
      );
      return;
    }
    ...
    const labelMap: Record<string, string> = {
      audio:
        lang === "de"
          ? "Generiere 60-Sekunden Audio-Briefing & Cloud TTS..."
          : "Generating 60-Second Audio Brief & Cloud TTS...",
      video:
        lang === "de"
          ? "Entwickle 9:16 Video-Storyboard mit Gemini..."
          : "Drafting 9:16 Video Storyboard with Gemini...",
      carousel:
        lang === "de"
          ? "Generiere redaktionelles Instagram-Karussell..."
          : "Synthesizing Editorial Carousel with Gemini...",
      newsletter:
        lang === "de"
          ? "Formuliere Executive Newsletter..."
          : "Drafting Executive Newsletter with Gemini...",
      factbox:
        lang === "de"
          ? "Extrahiere NZZ-Faktenbox-Kennzahlen..."
          : "Extracting Verified Fact Box...",
      faq:
        lang === "de"
          ? "Analysiere dialektisches FAQ (3 Perspektiven)..."
          : "Synthesizing Dialectical FAQ (3 Perspectives)...",
    };
    setGeneratingLabel(
      focusTab && labelMap[focusTab]
        ? labelMap[focusTab]
        : `Calling Google Cloud Vertex AI (${selectedModel})...`,
    );
    ...
    try {
      const result = await generateLiquidFormats({
        ...
        author:
          author.trim() || (lang === "de" ? "NZZ Redaktion" : "NZZ Editorial"),
      ...
      setErrorMessage(
        err.message ||
          "Generation failed with an unexpected error from Vertex AI.",
      );
    } finally {
  ...
  const handleUpdateSlide = (updatedSlide: CarouselSlide) => {
    ...
    const idx = slides.findIndex(
      (s) => s.slideNumber === updatedSlide.slideNumber,
    );
    if (idx >= 0) {
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-4 space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-800 pb-5">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-serif font-black tracking-tight text-white mt-1">
            {lang === "de"
              ? "Redaktioneller Entwurf & Multimodale Generierung"
              : "Live Article Draft & Real-Time Synthesis"}
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-12 space-y-1.5">
          <div className="flex items-center justify-between">
            <label className="block text-xs font-mono text-stone-400 uppercase tracking-wider">
              {lang === "de"
                ? "Titel / Schlagzeile (NZZ-Stil: kein Schlusspunkt)"
                : "Headline (NZZ Standard: No Terminal Period)"}
            </label>
            {/[.!?]$/.test(headline.trim()) && (
          ...
          <input
            ...
            placeholder={
              lang === "de"
                ? "Aussagekräftigen Titel eingeben..."
                : "Enter compelling investigative headline..."
            }
        ...
        <div className="lg:col-span-12 space-y-1.5">
          <label className="block text-xs font-mono text-stone-400 uppercase tracking-wider">
            {lang === "de"
              ? "Lead / Vorspann (optional)"
              : "Lead / Executive Summary (Optional)"}
          </label>
          <textarea
            rows={2}
            value={lead}
            ...
            placeholder={
              lang === "de"
                ? "Fassen Sie die Kernbotschaft in 1-2 prägnanten Sätzen zusammen..."
                : "Summarize the core premise in 1-2 dense sentences..."
            }
        ...
        <div className="lg:col-span-12 space-y-1.5">
          ...
          <textarea
            ...
            placeholder={
              lang === "de"
                ? "Fügen Sie hier Ihren vollständigen Recherchetext oder Entwurf ein..."
                : "Paste your full investigative draft, report, or analysis here..."
            }
        ...
        <div className="lg:col-span-12 bg-stone-900/80 border border-stone-800 rounded-2xl p-5 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-sm font-semibold text-white flex items-center gap-2">
              ...
              <span>
                {lang === "de"
                  ? "Multimodale Generierung auslösen"
                  : "Synthesize Multimodal Deliverables"}
              </span>
            </h2>
      ...
      {isGenerating && (
        <GenerationProgressBar isLoading={true} formatName={generatingLabel} />
      )}
        ...
        <div className="p-4 rounded-xl bg-red-950/80 border border-red-800 text-sm text-red-200 flex items-start gap-3 shadow-lg animate-fade-in">
          ...
          <div className="space-y-1">
            <h4 className="font-semibold text-red-300">
              {lang === "de"
                ? "Fehler bei der Vertex AI Kommunikation:"
                : "Vertex AI Communication Error:"}
            </h4>
        ...
        <div className="space-y-6 pt-4 border-t border-stone-800 animate-fade-in">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 bg-stone-950 border border-stone-800 rounded-xl p-4">
            <div className="flex flex-wrap items-center gap-3 text-xs">
              <span className="font-sans font-medium text-stone-400">
                Model Provenance:
              </span>
              <span
              ...
              <span className="text-stone-300 font-mono">
                ...
                <strong className="text-white">
                  {derivatives.model || selectedModel}
                </strong>
              </span>
            ...
            <div className="text-[11px] font-mono text-stone-500">
              Generated:{" "}
              {new Date(derivatives.generatedAt).toLocaleTimeString()}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center border-b border-stone-800 gap-1">
            ...
            {derivatives.visualVelocity &&
              derivatives.visualVelocity.charts?.length > 0 && (
                <button
            ...
            {activeTab === "audio" && derivatives.audioBrief && (
              <div className="max-w-2xl mx-auto space-y-6">
                ...
                <div className="bg-stone-950 border border-stone-800 rounded-xl p-5 space-y-3">
                  <h4 className="text-xs font-mono uppercase text-stone-400">
                    Synthesized Script (Studio Cadence)
                  </h4>
                  ...
                  <div className="pt-2 flex items-center gap-4 text-xs font-mono text-stone-500 border-t border-stone-800/80">
                    <span>
                      Voice:{" "}
                      {derivatives.audioBrief.voiceProfile?.voiceName ||
                        "Neural2"}
                    </span>
              ...
              <div className="space-y-4 max-w-5xl mx-auto">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-mono px-2.5 py-1 rounded bg-stone-900 text-stone-300 border border-stone-800">
                    9:16 Vertical •{" "}
                    {derivatives.socialStoryboard.totalDurationSeconds}s
                  </span>
                </div>
                  ...
                  <div className="bg-stone-950 border border-stone-800 rounded-2xl p-6 space-y-4 shadow-xl">
                    ...
                    <ul className="space-y-2.5 pt-2">
                      {derivatives.executiveNewsletter.bullets?.map((b, i) => (
                        <li
                          key={i}
                          className="text-xs text-stone-200 leading-relaxed flex items-start gap-2"
                        >
                  ...
                  <div className="bg-stone-950 border border-stone-800 rounded-2xl p-6 space-y-4 shadow-xl">
                    ...
                    <div className="space-y-3 pt-2">
                      {derivatives.factBox.metrics?.map((m) => (
                        <div
                          key={m.id}
                          className="p-3 bg-stone-900/60 border border-stone-800 rounded-xl flex items-center justify-between"
                        >
                          <div>
                          ...
                          <div className="text-right">
                            <span className="text-base font-serif font-bold text-white font-mono">
                              {m.value}
                            </span>
                            {m.delta && (
              ...
              <div className="space-y-6 max-w-4xl mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {derivatives.visualVelocity.charts?.map((c, idx) => (
                    <div
                      key={idx}
                      className="bg-stone-950 border border-stone-800 rounded-2xl p-6 space-y-4 shadow-xl"
                    >
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-serif font-bold text-white">
                          {c.title}
            ...
            {activeTab === "faq" && derivatives.dialecticalFaq && (
              <div className="max-w-3xl mx-auto space-y-4">
                ...
                {derivatives.dialecticalFaq.items?.map((item, idx) => (
                  <div
                    key={idx}
                    className="bg-stone-950 border border-stone-800 rounded-xl p-5 space-y-2 shadow-md"
                  >
                    <div className="flex items-center gap-2">
